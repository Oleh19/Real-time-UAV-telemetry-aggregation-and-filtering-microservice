...
- **Authenticated & hardened** — mutual TLS on the gRPC plane (every client presents a certificate signed by the project CA, TLS 1.3) plus bearer-token auth as a second factor, gRPC message-size limits, distroless read-only containers with dropped capabilities, and nginx security headers + rate limiting (see [Security](#security)).
...
- **Clustered message bus** — telemetry is split into one JetStream stream per geographic partition (`drone.telemetry.<p>`) plus an alerts stream, running on a 3-node NATS cluster so stream leaders spread across nodes and publishing parallelises across cores instead of serialising on one. `STREAM_STORAGE` (file/memory) and `STREAM_REPLICAS` trade durability for throughput. Consumers bind per-partition and fan in.
- **Durable pipeline** — samples flow through durable JetStream consumers: if a geofence worker is down, telemetry waits in the stream and is processed after restart, with zero loss.
- **Horizontal sharding (ingest, fusion, and geofence)** — the ingest+fusion tier scales out: the simulator routes every drone (and all the stations that see it) to one server instance by the drone's geographic cell, so cross-station fusion stays complete on a single instance and instance-prefixed track IDs never collide; each server publishes fused tracks partitioned by geographic cell onto `drone.telemetry.<partition>` subjects, and each geofence replica owns a disjoint slice of partitions via JetStream `FilterSubjects` (`SHARD_INDEX`/`SHARD_COUNT`), so replicas split the stream with no duplicated processing. A live-target aggregator on the API server consumes the whole fused stream to serve one unified air picture regardless of which instance fused a target, and cross-replica geofence state (oblast alarm presence, active swarms) is published to Postgres and served as a fresh union. Compose runs two server instances and two geofence replicas by default.
- **Sharded hot paths** — within each instance the fusion engine is split into geographically-sharded sub-fusers, each with its own lock and a fine spatial grid index (association is O(neighbours), not O(all tracks)); the classifier is striped by drone id and the station registry is lock-free on its hot path, so samples parallelise across cores instead of serialising on a single mutex.
...
- **Breach journal & incident workflow** — every zone entry and exit event flows through `drone.alerts` into a durable JetStream consumer that persists it to PostGIS; `GET /breaches` serves the log and the dashboard shows a live event feed where an operator can acknowledge and resolve each incident (`POST /breaches/{id}/ack|resolve`), with status tracked in the database.
...
- **Target classification** — every fused track is classified from its kinematic profile (mean speed and heading stability over a sliding window) into loitering munition, recon UAV, or multirotor; the class travels with the telemetry, shows in the target table and map tooltips, and per-class track counts are exported as metrics. The simulator spawns drones with matching distinct flight profiles.
- **Track quality & spoofing checks** — each track carries a 0–100 quality derived from the Kalman filter's position covariance, and the ingest path flags physically impossible jumps (>600 m/s) as possible spoofing (`uav_anomalies_total`), surfaced as a badge on the map and table.
- **Threat prioritization** — the dashboard ranks live targets by a composite threat score (class × speed × time-to-nearest-zone × track quality, boosted for spoofing) into a Threats panel, so the operator sees what to act on first.
...
- **Alerting & SLOs** — Prometheus recording rules track the ingest drop/success ratio and queue saturation as SLO series, and alert rules fire on a downed service, a stalled feed, drop rate above the 1% budget, success ratio under 95%, a saturated queue, publish failures, silent detection stations, and stalled history writes; Alertmanager receives them with severity-based grouping and inhibition (Alertmanager UI on `http://localhost:9093`, active alerts under Prometheus `/alerts`).
...
- **Mutual TLS** — the gRPC ingest requires client certificates signed by the project CA (TLS 1.3, `RequireAndVerifyClientCert`). A one-shot `certs` init service generates the CA, server, and client key pairs into a shared volume on first start (re-run `docker compose run --rm certs` after deleting the volume to rotate); the server, simulator, and replay publisher load them from `TLS_*` env vars. When those vars are unset the plane falls back to plaintext for local/test use and logs a warning.
- **Authenticated ingest** — on top of mTLS, `StreamTelemetry` is guarded by a bearer-token gRPC interceptor (constant-time comparison). The server rejects unauthenticated streams with `Unauthenticated` when `INGEST_TOKEN` is set; compose wires a shared dev token to server and simulator so local runs work out of the box. Override it via `.env` and never ship the default.
