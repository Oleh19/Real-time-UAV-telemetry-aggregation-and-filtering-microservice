...
- **Horizontal sharding** — the server partitions telemetry by geographic cell onto `drone.telemetry.<partition>` subjects; each geofence replica owns a disjoint slice of partitions via JetStream `FilterSubjects` (`SHARD_INDEX`/`SHARD_COUNT`), so N replicas split the stream with no duplicated processing. Cross-replica-aggregated state (oblast alarm presence and active swarms) is published to Postgres every second and the API serves the fresh union, so the air picture is consistent no matter which replica handled a given target. Compose runs two geofence replicas by default.
